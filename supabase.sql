...
-- Buyer history fields (safe to run on an existing project too).
...
begin
  ...
  set current_bid = p_amount, highest_bidder = trim(p_bidder_name), highest_bid_source = p_source, updated_at = now()
  where id = 1;
  insert into public.bids(item_number, item_name, bidder_name, bidder_email, bidder_address, amount, source)
  values (p_item_number, a.item_name, trim(p_bidder_name), null, nullif(trim(coalesce(p_bidder_address,'')), ''), p_amount, p_source);
  return jsonb_build_object('ok', true, 'message', 'Bid accepted.');
end;
$$;

grant execute on function public.place_bid(integer,text,numeric,text,text) to anon, authenticated;
